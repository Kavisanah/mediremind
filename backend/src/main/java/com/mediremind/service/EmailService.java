...
public class EmailService {
  ...
  public void sendWeeklyReport(String toEmail, String userName,
      long taken, long missed, String coachingTip) {
    sendHtml(toEmail,
        "MediRemind — Your Weekly Medicine Report 📊",
        buildWeeklyReportHtml(userName, taken, missed, coachingTip));
  }
  ...
  public void sendObserverWeeklyReport(List<String> observerEmails,
      String patientName, long taken, long missed, String coachingTip) {
    String subject = "MediRemind — Weekly Report for " + patientName + " 📊";
    String html = buildWeeklyReportHtml(patientName + " (your care recipient)", taken, missed, coachingTip);
    observerEmails.forEach(email -> sendHtml(email, subject, html));
  }
  ...
  private String buildWeeklyReportHtml(String name, long taken, long missed, String coachingTip) {
    ...
    String coachingTipHtml = "";
    if (coachingTip != null && !coachingTip.isBlank()) {
        coachingTipHtml = "<div style='background:#f0fafb;border-left:4px solid #1ea3a8;padding:16px;margin:20px 0;border-radius:0 12px 12px 0;'>"
            + "<h4 style='margin:0 0 6px 0;color:#106771;font-size:14px;'>💡 AI Coaching Tip</h4>"
            + "<p style='margin:0;font-size:14px;color:#374151;line-height:1.5;font-style:italic;'>\"" + coachingTip + "\"</p>"
            + "</div>";
    }
  ...
  public void sendPasswordResetEmail(String toEmail, String userName, String resetUrl) {
      String subject = "MediRemind — Reset your password 🔑";
      String html = wrap("🔑 Password Reset Request", "#8b5cf6",
              "<p style='font-size:16px;color:#374151;margin:0 0 20px;'>Hello <strong>" + userName + "</strong>,</p>"
                      + "<p style='color:#374151;margin:0 0 20px;'>We received a request to reset the password for your MediRemind account.</p>"
                      + "<table width='100%' cellpadding='0' cellspacing='0' style='margin:24px 0;'>"
                      + "<tr><td align='center'>"
                      + "<a href='" + resetUrl + "' style='display:inline-block;background:#8b5cf6;color:#ffffff;font-size:15px;font-weight:700;text-decoration:none;padding:12px 32px;border-radius:10px;box-shadow:0 4px 12px rgba(139,92,246,0.30);'>Reset Password</a>"
                      + "</td></tr>"
                      + "</table>"
                      + "<p style='color:#6b7280;font-size:13px;'>If you did not make this request, you can safely ignore this email. The link will expire in 1 hour.</p>");
      sendHtml(toEmail, subject, html);
  }
